...
import { CategorySchema, TCategorySchema } from "@/lib/shema"
...
import { addCategory, deleteCategory } from "@/app/api/actions"

interface Props {
  categories: CategoryGalleryPhoto[]
}
...
export const CategoryManager: React.FC<Props> = ({ categories }) => {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Вы уверены, что хотите удалить эту категорию? Все фотографии в этой категории могут стать недоступны.')) return
